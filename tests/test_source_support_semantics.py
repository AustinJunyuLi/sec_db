"""Tests for quote-level source-support helpers.

These tests pin the buyer-group rules: ``member_of`` only between
actors/groups, never with proposal descriptions; relation quotes must
literally name the subject actor.
"""
...
def test_member_of_quote_rejects_proposal_like_object() -> None:
    ...
    # member_of with a proposal description must fail even if both names appear.
